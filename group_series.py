...
def get_locals_from_xml(xml_path: str) -> set:
    ...
    for s in scans:
        if re.match(r"Localizer.*", s.get("type")) and s.find(f'{prefix}quality').text == "usable":
            locals.add(int(s.get("ID")))
    return sorted(locals)


# Read in the json for the BOLD runs from the bids dir
def get_func_from_bids(bids_path: str, locals: set[int], json_dict: defaultdict[list], groupings: list[dict[str:set]]):
    ...
    for jf in bids_func_json:
        ...
        for i, l in enumerate(locals):
            if i < len(locals)-1:
                if series_num > l and series_num < locals[i+1]:
...
def get_fmap_from_bids(bids_path: str, locals: set[int], json_dict: defaultdict[list], groupings: list[dict[str:set]]):
...
# Grouping field maps to BOLD task runs
def map_fmap_to_func(xml_path: str, bids_dir_path: str):
    print("####### Pairing field maps to functional runs #######")

    if not os.path.isfile(xml_path):
        exit_program_early(f"Session xml file {xml_path} does not exist.")
    # assert os.path.isfile(xml_path), "Session xml file does not exist"
    if not os.path.isdir(bids_dir_path):
        exit_program_early(f"Session bids dicrectory {bids_dir_path} does not exist.")
    # assert os.path.isdir(bids_dir_path), "Session bids directory does not exist"
